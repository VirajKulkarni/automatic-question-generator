...
class Encoder(nn.Module):
    def __init__(self, opt, dicts):
        ...
        self.bio_lut = nn.Embedding(8, 16, padding_idx=s2s.Constants.PAD)  # TODO: Fix this 
        self.feat_lut = nn.Embedding(64, 16, padding_idx=s2s.Constants.PAD)  # TODO: Fix this 
    ...
    def forward(self, input, bio, feats, hidden=None):
        ...
        print("------------------wordEMb input[0]",input[0])
        bioEmb = self.bio_lut(bio[0])
        print("bioemb bio[0]",input[0])
        featsEmb = [self.feat_lut(feat) for feat in feats[0]]
        print("featsEmb:",feats[0])
